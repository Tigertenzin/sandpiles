program sandpile                                                             ! Program for running NN BTW sandpile in 2D 
    ...
    character(len=1024) :: filename1                                        ! File name for the avalanches
    character(len=1024) :: filename2                                        ! File name for the avalanches
    character(len=1024) :: filename3                                        ! File name for the avalanches


    INTEGER, PARAMETER :: LONG=SELECTED_INT_KIND(18)                        ! Necessray variables for pseudo-random number generator
    INTEGER(LONG) :: seed
    REAL rnd48, r
    integer(kind=1) :: lambda01
    
    integer(kind=8) :: k=1,j=1, m=1, x=1,y=1, t=1                           ! variables for loop indices

    
    read *,timer, L, lambda, seed                                           ! Read in system parameters from console
    ...
    write(4,*) "seed", "SystemSize", "dissipation", "timer"
    write(4,*) seed, L, lambda, timer
    

    write (filename2, fmt='(a,I0,a)') "sp",seed,"_s.txt"          ! Test file of avalanche sizes
    open(unit=8,file=filename2)
    write(8,*) "seed", "SystemSize", "dissipation", "timer"
    write(8,*) seed, L, lambda, timer


    write (filename3, fmt='(a,I0,a)') "sp",seed,"_d.txt"          ! Test file of avalanche duration
    open(unit=9,file=filename3)
    write(9,*) "seed", "SystemSize", "dissipation", "timer"
    write(9,*) seed, L, lambda, timer
    ...
    do t = 1,1000000                                                ! Run system for transience, 1,000,000 avalanches
        call addGrainTransient(seed, L, lambda, lattice)
    end do
    ...
    do t = 1,timer
        call addGrain(seed, L, lambda, lattice)
    end do
...
Subroutine addGrain(seed, L, lambda, lattice)
    ...
    lattice(x,y) = lattice(x,y) + 1                                 ! Add single sand grain to (x,y)
    if (lattice(x,y) .gt. 4)  then                                  ! Check if driven site is above threshold, z_(x,y) > 4
    	call relaxLattice(seed, L, lambda, sizeS, durationD, lattice)          
...
Subroutine relaxLattice(seed, L, lambda, sizeS, durationD, lattice)
	    ...
	    do k=0, L-1
	        if (lattice(j,k) .gt. 4) then                               ! If a given site is above threshold...
	            ...
	            if (j .ne. 0) then                                      ! Left NN gains 1 (if not over boundary)
	                ...
	                toppleThese(L-1,k) = toppleThese(L-1,k) + 1 * lambda01(lambda,seed) ! If j=0, then neighbor is across the lattice at L-1. 
	            end if
	            
	            if (j .ne. L-1) then                                    ! Right NN gains 1 (if not over boundary)
	                toppleThese(j+1,k) = toppleThese(j+1,k) + 1 * lambda01(lambda,seed)
	            else 
	                toppleThese(0,k) = toppleThese(0,k) + 1 * lambda01(lambda,seed) ! If j=L-1, then neighbor is across the lattice at 0. 
	            end if
	            
	            if (k .ne. 0) then                                      ! Top NN gains 1 (if not over boundary)
	                toppleThese(j,k-1) = toppleThese(j,k-1) + 1 * lambda01(lambda,seed)
	            else 
	                toppleThese(j,L-1) = toppleThese(j,L-1) + 1 * lambda01(lambda,seed)
	            end if
	            
	            if (k .ne. L-1) then                                    ! Bottom NN gains 1 (if not over boundary)
	                toppleThese(j,k+1) = toppleThese(j,k+1) + 1 * lambda01(lambda,seed)
	            else
	                toppleThese(j,0) = toppleThese(j,0) + 1 * lambda01(lambda,seed)
	            end if
...
Subroutine addGrainTransient(seed, L, lambda, lattice)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Subroutine to decide on bulk dissipation (0-no, 1-yes)
